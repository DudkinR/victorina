        ...
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <textarea class="form-control" id="content" name="content" rows="3">{{$question->content}}</textarea>
                    </div>
                                        <!-- answer true   -->
                    <div class="form-group">
                    ...
                    @foreach($answers as $canswer)
                    <option value="{{$canswer->id}}"
                    @if(in_array($canswer->id,$correct_answers->pluck('id')->toArray()))
                         selected
                    @endif
                    >{{$canswer->content}}</option>
                    @endforeach
                    </select>
                    </div>
                    <!-- answer false   -->
                    ...
                    @foreach($answers as $uanswer)
                    <option value="{{$uanswer->id}}"
                    @if(in_array($uanswer->id,$uncorrect_answers->pluck('id')->toArray()))
                         selected
                    @endif
                    >{{$uanswer->content}}</option>
                    @endforeach
                    </select>
                    </div>
                      <!-- answer new  -->
